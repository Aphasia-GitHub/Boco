create or replace procedure get_value_manual_lc(v_time in varchar2)
is
ce1 number;
ce2 number;
cell_id1 number;
...
from c_tpd_cnt_bts_lc,c_cell
where c_tpd_cnt_bts_lc.int_id = c_cell.related_bts
and c_tpd_cnt_bts_lc.scan_start_time = to_date(v_time,'yyyy-mm-dd hh24:mi:ss')
...
and c.related_bts = c_bts.int_id and u.scan_start_time = to_date(v_time,'yyyy-mm-dd hh24:mi:ss')
...
for r1 in c1 loop
  ...
  wx_vol = cell_hwl2,
  city_id = city_id2
  where c_tpa_sts_cell.int_id = cell_id2 and scan_start_time = to_date(v_time,'yyyy-mm-dd hh24:mi:ss') ;
  ...
  wx_vol = cell_hwl1,
  city_id = city_id1
  where c_tpa_sts_cell.int_id = cell_id1 and scan_start_time = to_date(v_time,'yyyy-mm-dd hh24:mi:ss') ;
  end if;
  end loop;
end loop;
commit;

create or replace procedure Alarm_buhui(stat_time in date) is
...
data_tmp varchar2(64);
data_tmp1 varchar2(64);
...
--扇区退服告警的统计，用于数据的补汇,不是自动执行；
  ...
  commit;
  select 'c_tfa_alarm_act_history_'||to_char(to_date(stat_time,'yyyy-mm-dd hh24:mi:ss'),'yymm')  into data_tmp from dual;
  select 'c_tfa_alarm_act_history_'||to_char(last_day(to_date(stat_time,'yyyy-mm-dd hh24:mi:ss')+32),'yymm')  into data_tmp1 from dual;
  select to_char(to_date(stat_time,'yyyy-mm-dd hh24:mi:ss')) into stat_start_time from dual;
...
then round((to_date(to_char(to_date('''||stat_start_time||''',''yyyy-mm-dd hh24:mi:ss'')+23.9996/24,''yyyy-mm-dd hh24:mi:ss''),''yyyy-mm-dd hh24:mi:ss'')-c.event_time)*24*60,2)
 ...
 from c_bts a,c_region_city b, '||data_tmp||'  c
where a.city_id=b.city_id
and a.int_id=c.int_id
and c.object_class=201
and (c.org_type = 103 or c.omc_alarm_id in (132776, 201792, 133157))
...
then round((to_date(to_char(to_date('''||stat_start_time||''',''yyyy-mm-dd hh24:mi:ss'')+23.9996/24,''yyyy-mm-dd hh24:mi:ss''),''yyyy-mm-dd hh24:mi:ss'')-c.event_time)*24*60,2)
 ...
 from c_bts a,c_region_city b, '||data_tmp1||'  c
where a.city_id=b.city_id
and a.int_id=c.int_id
and c.object_class=201
and (c.org_type = 103 or c.omc_alarm_id in (132776, 201792, 133157))
...
s_error := sqlerrm;
rollback;
insert into job_log values(sysdate,'Alarm_buhui',s_error);
commit;

create or replace procedure get_value_zx
is
ce1 number;
ce2 number;
cell_id1 number;
...
and c_tpd_sts_bts.nbrAvailCe > 0
and c_tpd_sts_bts.scan_start_time = to_date(to_char(trunc(sysdate-1,'hh'),'yyyy-mm-dd hh24:mi:ss'),'yyyy-mm-dd hh24:mi:ss')
...
and c.related_bts = u.int_id and u.scan_start_time=to_date(to_char(trunc(sysdate-1,'hh'),'yyyy-mm-dd hh24:mi:ss'),'yyyy-mm-dd hh24:mi:ss')
...
for r1 in c1 loop
  ...
  wx_vol = cell_hwl2,
  city_id = city_id2
  where int_id = cell_id2 and scan_start_time = to_date(to_char(trunc(sysdate-1,'hh'),'yyyy-mm-dd hh24:mi:ss'),'yyyy-mm-dd hh24:mi:ss');
  ...
  wx_vol = cell_hwl1,
  city_id = city_id1
  where int_id = cell_id1 and scan_start_time = to_date(to_char(trunc(sysdate-1,'hh'),'yyyy-mm-dd hh24:mi:ss'),'yyyy-mm-dd hh24:mi:ss');
  end if;
  end loop;
end loop;
commit;
